...
namespace mosek
{
  ...
  int Task::appendconesseq(MSKconetypee ct, int numcone, int conesize, int firstvar)
  {
  ...
  int Task::appendcones(MSKconetypee ct, int numcone, int conesize, int firstvar, int d0, int d1)
  {
      int idx = getnumcone();
      ...
      {
          for (int i = 0; i < conesize; i++) subj[i] = d1*(i+k0*conesize)+firstvar;
          for (int k1 = 0; k1 < d1; ++k1)
          {
              MSK(appendcone, ct, 0.0, conesize, subj.data());
              for (int j = 0; j < conesize; ++j) subj[j] += d1;
          }
      }
  ...
  void Task::bound(MSKaccmodee acc, int idx, MSKboundkeye bk, double lb, double ub) { MSK(putbound, acc, idx, bk, lb, ub); }
  void Task::boundslice
    ( MSKaccmodee acc,
      int first,
      int last,
    ...
    const double    * cof)
  {
    ...
    //std::cout << __LINE__ << ": putarowslice: range : " << first << " : " << last << std::endl;
    //std::cout << "\tptrb = ";
  ...
  long long Task::appendsparsesymmat
    ( int            dim,
      size_t         num,
      const int    * subi,
      const int    * subj,
  ...
  void Task::getbarxj(MSKsoltypee sol, int j, double* barxj)
  {
    MSK(getbarxj,sol,j,barxj); }
  void Task::getbarsj(MSKsoltypee sol, int j, double* barxj)
  {
  ...
  void Task::revert(int numvar, int numcon, int numcone, int numbarvar)
  {
    ...
    {
      std::vector<int> subj(monty::range<int>(numcone,tasknumcone).reverse_begin(),
    ...
    {
      std::vector<int> subj(monty::range<int>(numbarvar,tasknumbarvar).reverse_begin(),
